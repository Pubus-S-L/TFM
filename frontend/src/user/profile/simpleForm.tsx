...
export function SimpleProfileForm({ profile, onSave }: SimpleProfileFormProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>, section: string | null = null, index: number | null = null, field: string | null = null) => {
    if (section && index !== null && field) {
      ...
      if (section === 'job') {
        setEditableProfile({
          ...editableProfile,
          job: { ...editableProfile.job, [field]: e.target.value } as Job
        });
      }
    } else {
  ...
  const addStudy = () => {
    setEditableProfile({
      ...editableProfile,
      studies: [...(editableProfile.studies || []), { degree: '', institution: '', graduationYear: '' }]
    });
  };
              ...
              <div className="grid gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleChange(e, 'studies', index, 'graduationYear')}
                  />
                </div>
              </div>
            </div>
