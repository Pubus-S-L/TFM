...
function ChatList() {
    ...
    useEffect(() => {
      ...
      fetch(`https://pubus.onrender.com/api/v1/chat/users/${currentUser.id}/chats`)
    ...
    const fetchUnreadMessages = (chatId) => {
      fetch(`https://pubus.onrender.com/api/v1/message/${chatId}/unread/${currentUser.id}`)
        .then(response => response.json())
        .then(data => {
    ...
    const handleSelectChat = (chatId) => {
      ...
      if (unreadMessages[chatId]) {
        fetch(`https://pubus.onrender.com/api/v1/message/${chatId}/markread/${currentUser.id}`, {
          method: 'PUT',
          headers: {
    ...
    useEffect(() => {
      chats.forEach(chat => {
        fetch(`https://pubus.onrender.com/api/v1/message/${chat.id}/messages`)
          .then(response => response.json())
          .then(data => {
            if (data.length > 0) {
    ...
    const handleSendMessage = () => {
      ...
      fetch(`https://pubus.onrender.com/api/v1/message/${selectedChatId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
