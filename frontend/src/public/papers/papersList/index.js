...
export default function Papers() {
  ...
  async function setUpTypes() {
    let types = await (
      await fetch(`https://pubus.onrender.com/api/v1/papers/types`, {
        headers: {
  ...
  async function setUp() {
    ...
    if (typesSelected.length > 0) {
      for(const paperType of typesSelected){
        ...
        let papersByType = await (
          await fetch(`https://pubus.onrender.com/api/v1/papers/types/${paperType}`, {
            headers: {
              "Content-Type": "application/json",
            },
          })
    ...
    let papersFiltered = await (
      await fetch(`https://pubus.onrender.com/api/v1/papers?search=${searchTerm}`, {
        headers: {
          "Content-Type": "application/json",
        },
      })
